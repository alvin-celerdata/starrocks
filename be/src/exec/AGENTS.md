...
- Do not add new `ExecEnv::GetInstance()` call sites under `be/src/exec`; prefer the query-scoped `ExecEnv` provided through `RuntimeState`. Including `exec/exec_env.h` is acceptable within the `Exec` compatibility target, but extracted lower-level targets must not depend on it.
