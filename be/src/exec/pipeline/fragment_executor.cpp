...
Status FragmentExecutor::_prepare_pipeline_driver(ExecEnv* exec_env, const UnifiedExecPlanFragmentParams& request) {
    ...
    PipelineGroupMap unready_pipeline_groups;
    _fragment_ctx->iterate_pipeline([&unready_pipeline_groups, runtime_state,
                                     driver_ctx = _fragment_ctx->driver_context()](Pipeline* pipeline) {
        ...
        pipeline->instantiate_drivers(runtime_state, driver_ctx);
    });
