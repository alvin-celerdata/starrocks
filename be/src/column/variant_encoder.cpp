    ...
    case TYPE_MAP: {
        ...
        for (uint32_t i = offset_data[row]; i < offset_data[row + 1]; ++i) {
            ...
            if (!key_datum.is_null()) {
                auto key_str = datum_to_string_checked(key_type_info.get(), key_datum);
                if (!key_str.ok()) {
                    return Status::VariantError("Unsupported variant map key type: " + key_type.debug_string());
                }
                ctx->keys.insert(std::move(key_str).value());
            }
...
#undef ENCODE_PRIMITIVE_CASE
    ...
    case TYPE_MAP: {
        ...
        for (uint32_t i = offset_data[row]; i < offset_data[row + 1]; ++i) {
            ...
            auto it = ctx->key_to_id.find(key_str.value());
            if (it == ctx->key_to_id.end()) {
                return Status::VariantError("Variant metadata missing field: " + key_str.value());
            }
