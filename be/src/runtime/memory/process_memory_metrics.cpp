...
void ProcessMemoryMetrics::install(MetricRegistry* registry, BeforeUpdateHook before_update_hook) {
    DCHECK(registry != nullptr);
    if (before_update_hook) {
        _before_update_hook = std::move(before_update_hook);
    }
...
void ProcessMemoryMetrics::_register_metrics(MetricRegistry* registry) {
...
#define SET_MEM_METRIC_VALUE(tracker, key)                                 \
    if (GlobalEnv::GetInstance()->tracker() != nullptr) {                  \
        key.set_value(GlobalEnv::GetInstance()->tracker()->consumption()); \
    }
